...
export default function Flood2({ navigation }) {
  ...
  const fetchWaterLevelData = async () => {
    const url = "http://192.168.43.170/aniban_2.php?data_type=water_level&last_id=0";
    try {
  ...
  const fetchForecastData = async () => {
    const forecastUrl = "http://192.168.43.170/testcode/aniban.php";
    try {
  ...
  const renderWaterLevelData = () => {
    console.log("Rendering water level data:", waterLevelData);
    ...
    return (
      ...
      >
        {waterLevelData.slice().reverse().map((entry, index) => {
  ...
  return (
    <View style={{ flex: 1, position: 'relative' }}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={{ height: 0, marginHorizontal: 5, marginTop: 30 }}>
          <TouchableOpacity
            onPress={() => navigation.navigate("Cam2")}
            style={{
              borderRadius: 9999,
              padding: 12,
              margin: 4,
        ...
        <View style={{ marginBottom: 10, marginTop: -50 }}>
          <View style={{ flexDirection: 'center', alignItems: 'center', marginHorizontal: 5, marginBottom: 4 }}>
            ...
            <View style={{ marginBottom: 10, marginTop: 25 }}>
              <Text style={{ color: 'lightblue', fontSize: 30 }}> Aniban </Text>
            </View>
